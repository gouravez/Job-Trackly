...
const TEST_STYLE = {
  null: "border-gray-200 dark:border-dark-border text-gray-600 dark:text-dark-tx2 hover:border-gray-300",
  sending:
    "border-gray-200 dark:border-dark-border text-gray-400 cursor-not-allowed",
  sent: "border-green-300 text-green-600 bg-green-50 dark:bg-green-900/20 dark:border-green-800 dark:text-emerald-400",
  empty:
    "border-yellow-300 text-yellow-600 bg-yellow-50 dark:bg-amber-900/20 dark:border-amber-800 dark:text-amber-400",
  error:
    "border-red-300 text-red-600 bg-red-50 dark:bg-red-900/20 dark:border-red-800 dark:text-red-400",
};
    ...
    return (
      <div className="space-y-5">
        {[1, 2].map((i) => (
          <div
            key={i}
            className="bg-white dark:bg-dark-s1 rounded-2xl border border-gray-100 dark:border-dark-border h-36 animate-pulse"
          />
        ))}
      </div>
    );
  ...
  return (
    <div className="space-y-5">
      <SettingsCard
        ...
        description="Get emailed when applications haven't moved in a while"
      >
        <ToggleRow
      ...
      {settings.reminderEnabled && (
        <SettingsCard
          ...
          description="Customise when and how often you hear from us"
        >
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-3">
              <input
                ...
                className="flex-1 accent-dark-accent"
              />
              <span className="text-sm font-semibold text-dark-accent w-20 text-right">
                {settings.reminderDays} day
                {settings.reminderDays !== 1 ? "s" : ""}
              </span>
            </div>
            <p className="text-xs text-gray-400 dark:text-dark-tx2">
              Applications that haven't been updated in this many days will
              trigger a reminder.
            </p>
          </div>
                ...
                <button
                  ...
                  className={cn(
                    ...
                    settings.reminderFrequency === freq
                      ? "border-dark-accent bg-[#eef2ff] dark:bg-dark-blue-tint text-dark-accent"
                      : "border-gray-200 dark:border-dark-border text-gray-500 dark:text-dark-tx2 hover:border-gray-300",
                  )}
                >
                  {freq}
                </button>
          ...
          <div className="pt-1">
            <p className="text-xs text-gray-400 dark:text-dark-tx2 mb-3">
              Want to see what the email looks like? Send yourself a live
              preview.
            </p>
            <button
              onClick={handleTest}
