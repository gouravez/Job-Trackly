import { TrendingUp, Calendar, Star, X } from 'lucide-react'
import useAppStore from '@/store/appStore'

export default function DashboardStatCards() {
  const applications = useAppStore((s) => s.applications)

  const STATS = [
    { label: 'Total Applications', value: applications.length,
      sub: '+12% this month', icon: TrendingUp, iconBg: 'bg-blue-50 dark:bg-blue-900/30',  iconColor: 'text-blue-500'  },
    { label: 'Interviews', value: applications.filter((a) => a.status === 'Interview').length,
      sub: '+4% this month',  icon: Calendar,   iconBg: 'bg-green-50 dark:bg-green-900/30', iconColor: 'text-green-500' },
    { label: 'Offers',     value: applications.filter((a) => a.status === 'Offer').length,
      sub: '+1 this month',   icon: Star,       iconBg: 'bg-amber-50 dark:bg-amber-900/30', iconColor: 'text-amber-500' },
    { label: 'Rejections', value: applications.filter((a) => a.status === 'Rejected').length,
      sub: '+8% this month',  icon: X,          iconBg: 'bg-red-50 dark:bg-red-900/30',     iconColor: 'text-red-400'   },
  ]

  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
      {STATS.map((s) => (
        <div key={s.label} className="bg-white dark:bg-gray-900 rounded-2xl p-5 border border-gray-100 dark:border-gray-800 shadow-sm">
          <div className="flex items-center justify-between mb-3">
            <p className="text-sm text-gray-500 dark:text-gray-400">{s.label}</p>
            <div className={`w-9 h-9 rounded-xl ${s.iconBg} flex items-center justify-center`}>
              <s.icon size={18} className={s.iconColor} />
            </div>
          </div>
          <p className="text-4xl font-extrabold text-gray-900 dark:text-white">{s.value}</p>
          <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">{s.sub}</p>
        </div>
      ))}
    </div>
  )
}