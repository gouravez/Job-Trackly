...
export default function DashboardStatCards() {
  ...
  const stats = useMemo(() => {
    ...
    const thisMonthApps = applications.filter(
      (a) => getMonthKey(a) === thisMonth,
    );
    const lastMonthApps = applications.filter(
      (a) => getMonthKey(a) === lastMonth,
    );
    ...
    return [
      {
        ...
        sub: formatDelta(
          count(thisMonthApps, isAny),
          count(lastMonthApps, isAny),
        ),
        positive: count(thisMonthApps, isAny) >= count(lastMonthApps, isAny),
        icon: TrendingUp,
        iconBg: "bg-blue-50 dark:bg-dark-blue-tint",
        iconColor: "text-blue-500 dark:text-dark-accent3",
      },
      {
        label: "Interviews",
        value: count(applications, isInterview),
        sub: formatDelta(
          count(thisMonthApps, isInterview),
          count(lastMonthApps, isInterview),
        ),
        positive:
          count(thisMonthApps, isInterview) >=
          count(lastMonthApps, isInterview),
        icon: Calendar,
        iconBg: "bg-green-50 dark:bg-dark-green-tint",
      ...
      },
      {
        ...
        sub: formatDelta(
          count(thisMonthApps, isOffer),
          count(lastMonthApps, isOffer),
        ),
        positive:
          count(thisMonthApps, isOffer) >= count(lastMonthApps, isOffer),
        icon: Star,
        iconBg: "bg-amber-50 dark:bg-dark-amber-tint",
      ...
      },
      {
        ...
        sub: formatDelta(
          count(thisMonthApps, isRejected),
          count(lastMonthApps, isRejected),
        ),
        positive:
          count(thisMonthApps, isRejected) <= count(lastMonthApps, isRejected),
        icon: X,
        iconBg: "bg-red-50 dark:bg-dark-red-tint",
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
      {stats.map((s) => (
        <div
          key={s.label}
          className="bg-white dark:bg-dark-s1 rounded-2xl p-5 border border-gray-100 dark:border-dark-border shadow-sm dark:shadow-none"
        >
          ...
          <p className="text-4xl font-extrabold text-gray-900 dark:text-dark-tx1">
            {s.value}
          </p>
          <p
