...
export default function MonthlyTrendsChart() {
  ...
  const chartData = useMemo(() => {
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      data.push({ month: MONTHS[d.getMonth()], year: d.getFullYear(), sent: 0, responses: 0 })
    }
  ...
  return (
    <div className="col-span-3 bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm p-6">
      <div className="flex items-center justify-between mb-4">
        <h3 className="font-bold text-gray-900 dark:text-white">Monthly Application Trends</h3>
        <div className="flex items-center gap-4 text-xs text-gray-500">
          <div className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-[#2f54c8]" /> Applications Sent</div>
          <div className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-teal-500" /> Responses Received</div>
        </div>
      </div>
      <ResponsiveContainer width="100%" height={220}>
        <BarChart data={chartData} margin={{ left: -20, right: 0, top: 5, bottom: 0 }} barGap={4}>
