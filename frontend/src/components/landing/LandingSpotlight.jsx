...
import {
  ...
  AnalyticsMockup,
  DashboardMockup,
} from "@/components/landing/LandingMockups.jsx";
  ...
  return (
    <div className="grid md:grid-cols-2 gap-16 items-center">
      <Reveal variant={reveal} className={textOrder}>
        <p
          className={`text-sm font-semibold ${eyebrowColor} mb-3 tracking-widest uppercase`}
        >
          {eyebrow}
        </p>
        ...
        <ul className="space-y-2">
          {items.map((item) => (
            ...
            >
              <CheckCircle2 size={16} className={`${dotColor} flex-shrink-0`} />{" "}
              {item}
            </li>
          ))}
        </ul>
      ...
      <Reveal
        variant={mockupReveal}
        className={`flex justify-center ${mockOrder}`}
      >
        {mockup}
      </Reveal>
    </div>
  ...
  return (
    <section className="max-w-5xl mx-auto px-6 py-24 space-y-24">
      ...
      <SpotlightRow
        ...
        items={[
          "Stat cards for applied, interviews, offers, and rejected",
          "30-day application activity trend",
          "Recent applications at a glance",
          "Follow-ups due, surfaced automatically",
        ]}
