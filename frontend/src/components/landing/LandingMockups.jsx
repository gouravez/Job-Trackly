...
export function DashboardMockup() {
  ...
  return (
    <div
      className="rounded-xl border border-gray-200 bg-white shadow-lg p-4 w-full max-w-xs"
      data-cursor-hover
    >
      ...
      <div className="space-y-1.5 mb-3">
        {recent.map((r) => (
          <div key={r.role} className="flex items-center gap-2 text-[10px]">
            <span
              className={`w-1.5 h-1.5 rounded-full ${r.dot} flex-shrink-0`}
            />
    ...
    >
      ...
      <div className="grid grid-cols-7 gap-1">
        {cells.map((d, i) => (
          <div
            key={i}
            className={`aspect-square flex items-center justify-center rounded-md text-[11px] font-medium relative ${d === 14 ? "bg-blue-500 text-white" : "text-gray-700"}`}
          >
            {d || ""}
            {d && dotColor[d] && (
              <span
                className={`absolute bottom-0.5 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full ${dotColor[d]}`}
              />
            )}
          </div>
        ))}
      ...
      <div className="mt-3 space-y-1.5">
        {[
          {
            color: "bg-teal-400",
            label: "Interview — Google SWE",
            day: "Jun 8",
          },
          { color: "bg-amber-400", label: "Follow-up — Stripe", day: "Jun 15" },
        ].map((e) => (
          <div key={e.label} className="flex items-center gap-2 text-[10px]">
            <span
              className={`w-1.5 h-1.5 rounded-full ${e.color} flex-shrink-0`}
            />
    ...
    >
      ...
      <div className="flex items-end gap-1.5 h-20 mb-2">
        {bars.map((h, i) => (
          <div
            key={i}
            className="flex-1 rounded-t-sm bg-blue-100 flex flex-col justify-start"
            style={{ height: `${h}%` }}
          >
            <div
              className="rounded-t-sm bg-blue-500 w-full"
              style={{ height: "30%" }}
            />
          </div>
        ))}
      ...
      <div className="pt-3 border-t border-gray-100 flex gap-3">
        {[
          { label: "Applied", val: "12", c: "text-blue-600" },
          { label: "Interviews", val: "4", c: "text-teal-600" },
          { label: "Response rate", val: "33%", c: "text-green-600" },
        ].map((s) => (
