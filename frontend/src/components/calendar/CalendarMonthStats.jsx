...
export default function CalendarMonthStats({
  year,
  month,
  applications,
  eventMap,
  ...
  const stats = {
    ...
    interviews: applications.filter(
      (a) => a.status === "Interview" && a.updatedAt?.startsWith(prefix),
    ).length,
    followUps: Object.keys(eventMap).filter(
      (k) =>
        k.startsWith(prefix) &&
        eventMap[k].some((e) => e.eventType === "FollowUp"),
    ).length,
    offers: applications.filter(
      (a) => a.status === "Offer" && a.updatedAt?.startsWith(prefix),
    ).length,
  };

  return (
    <div className="bg-white dark:bg-dark-s1 rounded-2xl border border-gray-100 dark:border-dark-border shadow-sm p-5">
      <h3 className="font-bold text-gray-900 dark:text-dark-tx1 text-sm mb-3">
        ...
        {STATS_CONFIG.map(({ key, label, color }) => (
          <div key={key} className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              />
              <span className="text-xs text-gray-500 dark:text-dark-tx2">
                {label}
              </span>
            </div>
