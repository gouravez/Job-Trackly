...
import {
  LayoutDashboard, BarChart2, Bell, FileText, Zap, ShieldCheck,
  ArrowRight, Play,
} from 'lucide-react'
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <p className="text-sm text-gray-600">
        Join <span className="font-bold text-gray-900">2,400+ students</span> staying organized
      </p>
    </div>
  )
}
    ...
    <div className="rounded-2xl border border-gray-200 shadow-2xl overflow-hidden bg-white">
      ...
      <div className="p-5 bg-gray-50 min-h-[280px]">
        <div className="flex gap-3 overflow-x-auto">
          {[
            { col: 'Applied', color: 'bg-blue-100 text-blue-700', count: 12, cards: ['Google SWE', 'Meta PM', 'Stripe DS'] },
            { col: 'Interviewing', color: 'bg-amber-100 text-amber-700', count: 4, cards: ['Airbnb Design', 'Notion PM'] },
            { col: 'Offer', color: 'bg-green-100 text-green-700', count: 1, cards: ['Linear Eng'] },
            { col: 'Rejected', color: 'bg-gray-100 text-gray-500', count: 3, cards: ['Twitter', 'Netflix'] },
          ].map((col) => (
            <div key={col.col} className="flex-shrink-0 w-44">
              <div className="flex items-center justify-between mb-2">
                <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${col.color}`}>{col.col}</span>
                <span className="text-xs text-gray-400">{col.count}</span>
              </div>
              <div className="space-y-2">
                {col.cards.map((card) => (
                  <div key={card} className="bg-white rounded-lg p-2.5 shadow-sm border border-gray-100">
                    <div className="text-xs font-medium text-gray-800">{card}</div>
                    <div className="text-[10px] text-gray-400 mt-0.5">Updated 2d ago</div>
                  </div>
                ))}
              </div>
            </div>
    ...
    <div className="min-h-screen bg-white font-sans">
      ...
      <section className="max-w-4xl mx-auto px-6 pt-20 pb-16 text-center">
        ...
        <h1 className="text-5xl md:text-6xl font-extrabold text-gray-900 leading-[1.1] tracking-tight mb-6">
          Stay on top of every job<br />application you send.
        </h1>

        <p className="text-lg text-gray-500 max-w-xl mx-auto mb-10 leading-relaxed">
          AppTrack is your personal workspace to organize applications,
      ...
      <section id="features" className="max-w-5xl mx-auto px-6 py-24">
        <div className="text-center mb-16">
          ...
          <p className="text-gray-500 max-w-xl mx-auto">
            No recruiters, no noise — just the tools you need to manage your own applications with clarity.
          </p>
        </div>
      ...
      <section className="px-6 pb-24">
        <div className="max-w-4xl mx-auto bg-gray-900 rounded-3xl px-10 py-16 text-center">
          ...
          <p className="text-gray-400 mb-10 max-w-md mx-auto">
            Free forever for students. No credit card required. Set up your workspace in under a minute.
          </p>
      ...
      <footer className="border-t border-gray-100 px-6 py-8">
        <div className="max-w-6xl mx-auto flex flex-wrap items-center justify-between gap-4">
          ...
          <p className="text-sm text-gray-400">© 2025 AppTrack</p>
        </div>
      </footer>
    </div>
  )
