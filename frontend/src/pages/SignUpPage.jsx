...
import { Mail, Lock, Eye, EyeOff, User, ArrowLeft, RefreshCw } from 'lucide-react'
  ...
  return (
    <div className="w-full max-w-[440px] space-y-6">
      <div className="space-y-1">
        <Logo size="lg" />
        <div className="pt-3">
          <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Create your free account</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Start organizing your job search in minutes</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex gap-1 flex-1">
                {[1,2,3,4].map((n) => (
                  <div key={n} className={cn('h-1 flex-1 rounded-full transition-all duration-300',
                    strength.score >= n
                      ? strength.score <= 1 ? 'bg-red-400' : strength.score === 2 ? 'bg-amber-400'
                        : strength.score === 3 ? 'bg-blue-400' : 'bg-green-500'
        ...
        <div className="space-y-2">
          <label className="text-sm font-semibold text-gray-800 dark:text-gray-200">Where are you in your job search?</label>
...
function OtpStep({ formData, onBack }) {
  ...
  return (
    <div className="w-full max-w-[440px] space-y-6">
      ...
      <div>
        <button onClick={onBack} className="flex items-center gap-1.5 text-sm text-gray-500 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200 mb-5 transition-colors">
          <ArrowLeft size={15} /> Back
        </button>
        <h2 className="text-2xl font-bold text-gray-900 dark:text-white">Check your email</h2>
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
          We sent a 6-digit code to <span className="font-semibold text-gray-800 dark:text-gray-200">{formData.email}</span>
        </p>
      </div>
            ...
            <input
              ...
              className={cn(
                ...
                digit
                  ...
                  : 'border-gray-200 dark:border-gray-700 focus:border-[#2f54c8]'
              )}
            />
          ))}
        </div>
      ...
      <div className="text-center text-sm text-gray-500 dark:text-gray-400">
        ...
        {countdown > 0 ? (
          ...
          <button onClick={handleResend} disabled={resending}
            className="text-[#2f54c8] font-semibold hover:underline inline-flex items-center gap-1 disabled:opacity-50">
